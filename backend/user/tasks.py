...
def send_welcome_email_task(self, user_id):
    try:
        ...
        html_message = render_to_string(
            'user/welcome_email.html', {'username': user.username})
        ...
        try:
            ...
            print(
                f"[Celery Task] Error sending email to {user.email}: {exc}. Retrying...")
            raise self.retry(exc=exc)

    except User.DoesNotExist:
        print(
            f"[Celery Task] User with id {user_id} does not exist. Cannot send email.")
