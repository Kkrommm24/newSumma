...
class RequestPasswordResetView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            payload = {
                'user_id': str(user.pk),
                'exp': datetime.now(timezone.utc) + PASSWORD_RESET_JWT_LIFETIME,
                'token_type': 'password_reset'
            }
            
            # Ký token với secret key
            reset_token_str = jwt.encode(payload, settings.SECRET_KEY, algorithm='HS256')
...
class PasswordResetConfirmView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            logger.info(f"Attempting to reset password with token: {reset_token_str[:10]}...")

            try:
                payload = jwt.decode(reset_token_str, settings.SECRET_KEY, algorithms=['HS256'])
                ...
                tokens.delete()
                logger.info(f"Blacklisted and deleted old tokens for user: {user.email}")
                ...
                return Response({'error': 'Liên kết đặt lại mật khẩu đã hết hạn.'}, status=status.HTTP_400_BAD_REQUEST)
            except jwt.InvalidTokenError as e:
                logger.error(f"Invalid token error: {str(e)}")
                return Response({'error': f'Liên kết đặt lại mật khẩu không hợp lệ: {str(e)}'}, status=status.HTTP_400_BAD_REQUEST)
        except User.DoesNotExist:
            ...
            if hasattr(e, 'detail'):
                if isinstance(e.detail, dict):
                    ...
                    return Response({'error': error_messages[0] if error_messages else str(e)}, status=status.HTTP_400_BAD_REQUEST)
