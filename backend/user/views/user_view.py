...
class UserRegistrationView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            is_conflict = False
            if 'email' in error_detail and any('already exists' in str(msg) for msg in error_detail['email']):
                is_conflict = True
            if 'username' in error_detail and any('already exists' in str(msg) for msg in error_detail['username']):
                is_conflict = True
        ...
        send_welcome_email_task.apply_async(args=[str(user.id)], queue='high_priority')
