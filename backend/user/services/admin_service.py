...
class AdminService:
    def get_system_stats(self):
        ...
        total_comments = ArticleStats.objects.aggregate(
            total=Sum('comment_count'))['total'] or 0
        total_views = ArticleStats.objects.aggregate(
        ...
        new_articles_24h = NewsArticle.objects.filter(
            created_at__gte=last_24h).count()
        new_summaries_24h = NewsSummary.objects.filter(
            created_at__gte=last_24h).count()
    ...
    def get_source_stats(self):
        ...
        for source in sources:
            ...
            summary_count = NewsSummary.objects.filter(
                article_id__in=NewsArticle.objects.filter(
                    source_id=source.id).values_list(
                    'id', flat=True)).count()
    ...
    def get_users_by_keyword(self, keyword):
        ...
        try:
            ...
            preferences = UserPreference.objects.filter(
                favorite_keywords__contains=[keyword])
            ...
            users = User.objects.filter(
                id__in=user_ids).order_by('-created_at')
    ...
    def get_all_articles(self, filters=None, ordering=None):
        ...
        if filters:
            ...
            if 'source_name' in filters:
                source_ids = NewsSource.objects.filter(
                    name__in=filters['source_name']).values_list(
                    'id', flat=True)
        ...
        if ordering:
            ...
            if field == 'source_name':
                ...
                queryset.sort(
                    key=lambda x: source_dict.get(
                        x.source_id, ''), reverse=reverse)
    ...
    def get_all_summaries(self, filters=None, ordering=None):
        ...
        if filters:
            if 'article_title' in filters:
                article_ids = NewsArticle.objects.filter(
                    title__in=filters['article_title']).values_list(
                    'id', flat=True)
        ...
        if ordering:
            ...
            if field == 'article_title':
                ...
                reverse = ordering.startswith('-')
                queryset.sort(
                    key=lambda x: article_dict.get(
                        x.article_id, ''), reverse=reverse)
            elif field in valid_fields:
                queryset = queryset.order_by(ordering)
            else:
                queryset = queryset.order_by('-created_at')
    ...
    def get_all_comments(self, filters=None, ordering=None):
        ...
        if filters:
            if 'username' in filters:
                user_ids = User.objects.filter(
                    username__in=filters['username']).values_list(
                    'id', flat=True)
        ...
        if ordering:
            ...
            if field == 'username':
                ...
                reverse = ordering.startswith('-')
                queryset.sort(
                    key=lambda x: user_dict.get(
                        x.user_id, ''), reverse=reverse)
    ...
    def get_all_favorite_words(self, filters=None, ordering=None):
        ...
        if ordering:
            ...
            if field == 'keyword':
                favorite_words.sort(
                    key=lambda x: x['keyword'], reverse=reverse)
                ...
                favorite_words.sort(
                    key=lambda x: x['user_count'], reverse=True)
        else:
            # Mặc định sắp xếp theo số người dùng giảm dần
