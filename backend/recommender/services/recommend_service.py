...
def _trigger_category_ranking_update_for_related_items(user_id, interacted_summary_id):
    if not user_id:
        return

    try:
        interacted_summary = get_object_or_404(NewsSummary, id=interacted_summary_id)
        article_id = interacted_summary.article_id
        if not article_id:
            return

        article_category_relation = NewsArticleCategory.objects.filter(article_id=article_id).first()
        ...
        logger.warning(f"Interacted summary with id {interacted_summary_id} not found for triggering related item ranking update.")
    except Exception as e:
        logger.error(f"Error triggering category ranking update for related items of summary {interacted_summary_id} for user {user_id}: {e}", exc_info=True)
...
def log_summary_view(user_id, summary_id, duration_seconds):
    ...
    try:
        ...
        if duration_seconds >= VIEW_DURATION_THRESHOLD and actual_user_id:
            _trigger_category_ranking_update_for_related_items(actual_user_id, summary.id)
...
def handle_summary_click_for_ranking(user_id, summary_id):
    try:
        ...
        if user_id: # Only update ranking if user is authenticated
            _trigger_category_ranking_update_for_related_items(user_id, summary.id)
...
def get_recommendations_for_user(user_id, current_summary_id=None, limit=10, offset=0):
    if not user_id:
        return [], {}, {
            'type': 'auth_required',
    ...
    try:
        # Hoàn lại cách query NewsSummary ban đầu, không dùng select_related('article') dựa trên FK không tồn tại
        ...
        # Tuy nhiên, logic xếp hạng/tạo ranking mới cần tất cả summaries tiềm năng
        ...
        for summary in summaries_qs: # Lặp qua tất cả summaries tiềm năng để tính/lấy điểm
            ...
            if ranking:
                ...
                try:
                    ...
                    logger.error(f"Error creating ranking for summary {summary.id}: {e_create_ranking}", exc_info=True)
                    summary.score = 0 
        ...
        sorted_summaries = sorted(
            summaries_with_score,
            key=lambda x: (x.score, getattr(x, 'created_at', timezone.now())), # Thêm getattr để tránh lỗi nếu created_at thiếu
            reverse=True
        )
    ...
    try:
        ...
        for summary in summaries:
            try:
                ...
                category_score = 0.0
                article_categories = NewsArticleCategory.objects.filter(article_id=summary.article_id)
                if article_categories.exists():
                    for category in article_categories:
                        ...
                        category_score = max(category_score, min(category_views / 10, 1.0))
                ...
                if user_id:
                    ...
                    if search_history_keywords:
                        ...
                        fts_queries = [SearchQuery(kw, search_type='plain', config=search_config) for kw in search_history_keywords if kw]
                        if fts_queries:
                ...
                if user_id:
                    try:
                        ...
                        if user_pref.favorite_keywords:
                            ...
                            search_config = 'vietnamese'
                            fts_queries = [SearchQuery(kw, search_type='plain', config=search_config) for kw in fav_kws if kw]
                            if fts_queries:
                                combined_fts_query = reduce(operator.or_, fts_queries)
                                summary_rank_obj = NewsSummary.objects.filter(pk=summary.pk).annotate(
                                    rank=SearchRank(F('search_vector'), combined_fts_query)
