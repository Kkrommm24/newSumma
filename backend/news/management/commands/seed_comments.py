    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(
                    "🔄  Đang reset comment_count trong ArticleStats về 0 cho các bài viết có comment...")
                ...
                if not users:
                    self.stdout.write(
                        self.style.WARNING("⚠️ Không tìm thấy User nào. Bỏ qua seed comments."))
                    return

                if not articles:
                    self.stdout.write(
                        self.style.WARNING("⚠️ Không tìm thấy NewsArticle nào. Bỏ qua seed comments."))
                    return

                self.stdout.write(self.style.NOTICE(
                    f"⏳ Seeding comments cho {len(articles)} bài viết và {len(users)} người dùng..."))

                comments_to_create = []
                num_comments_to_seed = 50
                article_comment_counts = {
                    article.id: 0 for article in articles}
                ...
                for i in range(num_comments_to_seed):
                    ...
                    content = template.format(
                        topic=random.choice(topics), user=user.username)
                    ...
                    if (i + 1) % 10 == 0:
                        self.stdout.write(
                            f"   ➕ Đã chuẩn bị {i + 1}/{num_comments_to_seed} comments...")

                Comment.objects.bulk_create(comments_to_create)
                self.stdout.write(self.style.SUCCESS(
                    f"✔ Đã tạo thành công {len(comments_to_create)} comments."))

                self.stdout.write(
                    "🔄  Đang cập nhật comment_count trong ArticleStats...")
                ...
                self.stdout.write(self.style.SUCCESS(
                    f"✔ Đã cập nhật comment_count cho {articles_updated_stats} bài viết trong ArticleStats."))

        except IntegrityError as e:
            self.stderr.write(self.style.ERROR(
                f"❌ Lỗi IntegrityError khi seed comments (có thể do article_id hoặc user_id không hợp lệ): {str(e)}"))
        except Exception as e:
            self.stderr.write(self.style.ERROR(
                f"❌ Lỗi khác khi seed comments: {str(e)}"))
