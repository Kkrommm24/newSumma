...
def save_articles_with_categories(source_name, source_url, articles):
    ...
    try:
        ...
        category_names = {article.get(
            "category_name") for article in articles if article.get("category_name")}
        ...
        if missing_categories:
            logger.error(
                f"❌ Lỗi: Các category sau không tồn tại trong DB: {missing_categories}. Không thể lưu bài viết.")
        ...
        for article in articles:
            ...
            if not url or url in valid_article_urls:
                logger.warning(
                    f"⚠️ Bỏ qua bài viết bị thiếu URL hoặc URL trùng lặp trong batch: {url}")
                continue
            ...
            if not category_obj:
                logger.warning(
                    f"⚠️ Bỏ qua bài viết vì không tìm thấy category '{category_name}' (đã kiểm tra trước đó?): {article.get('title')}")
                continue

            article_id = uuid.uuid4()
            published_time = parse_datetime(article.get("published_at")) if article.get(
                "published_at") else timezone.now()
            ...
            try:
                with transaction.atomic():
                    created_articles = NewsArticle.objects.bulk_create(
                        articles_to_create, ignore_conflicts=True)
                    if article_category_relations_to_create:
                        NewsArticleCategory.objects.bulk_create(
                            article_category_relations_to_create, ignore_conflicts=True)

                    saved_count = len(created_articles)
                    logger.info(
                        f"💾 Đã bulk_create {saved_count} bài viết và gán category.")
        ...
        logger.exception(
            f"❌ Lỗi nghiêm trọng trong save_articles_with_categories: {e}")
        return 0
