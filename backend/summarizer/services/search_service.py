...
def search_summaries_with_articles(query_string):
    try:
        search_query = SearchQuery(query_string, config='vietnamese', search_type='websearch')
        ...
        logger.error(f"Error during summary search service for query '{query_string}': {e}", exc_info=True)
