...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            type=int,
            default=10,
            help='Số lượng bài viết cần tóm tắt')
        parser.add_argument(
            '--verbose',
            action='store_true',
        ...
        parser.add_argument(
            '--check-gpu',
            action='store_true',
            help='Kiểm tra GPU trước khi chạy')
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if verbose:
                ...
                self.stdout.write(
                    f"Tổng thời gian thực thi: {total_time:.2f} giây")
    ...
    def _check_gpu_status(self):
        ...
        try:
            result = subprocess.run(
                ['nvidia-smi'], stdout=subprocess.PIPE, text=True)
            self.stdout.write(result.stdout)
        except FileNotFoundError:
            self.stdout.write(
                "nvidia-smi không được cài đặt hoặc không tìm thấy")

        # NVCC version
        self.stdout.write("\nThông tin NVCC:")
        try:
            result = subprocess.run(
                ['nvcc', '--version'], stdout=subprocess.PIPE, text=True)
            self.stdout.write(result.stdout)
        except FileNotFoundError:
